...
// Hopefully we'll migrate out of this soon, but for the sake of focusing on the port
// Decided not to try to refactor this already

// Hopefully also migrating out of lodash probably (investigate lodash/fp and similar maybe)
// but added it simply for the sake of speeding up the process
...
export function lessonsByDay(state) {
    const selectedCourses = _selectedCourses(state);
...
export function selectedCourses(state) {
    const selectedCourses = _selectedCourses(state);
    if (selectedCourses === null) return null;
    const lessons = flatten(Object.values(lessonsByDay(state)));
    ...
    return selectedCourses.map((course) => ({
        ...
        classes: course.classes.map((c) => ({
            ...
            description: c.teacherAcronym
                ? [c.className, c.teacherAcronym, `${lang(state)[`DAY_SHORT${c.day + 2}`]} ${c.timeStart}`].join(" - ")
                : c.className,
        })),
    }));
}
...
export const locationHash = (state) => {
    let string = `#${state.selectedYear}!${state.selectedSemester}`;
    const programmesCourses = {};
    const currSelectedCourses = selectedCourses(state);
    if (!currSelectedCourses || currSelectedCourses.length === 0) return "#";
    currSelectedCourses.forEach((course) => {
